...
class _DigitalSignatureScreenState extends State<DigitalSignatureScreen> {
  ...
  bool get _hasVisibleSignatureStroke {
    ...
    for (final point in _signaturePoints) {
      if (point == null) {
        previousPoint = null;
          ...
          (point - previousPoint).distanceSquared > 4) {
        return true;
      } else {
        previousPoint = point;
      }
  ...
  void _addSignaturePoint(Offset point) {
    setState(() {
      ...
      _signatureCaptured = _hasVisibleSignatureStroke;
    });
  }
  ...
  String _buildSignatureDataUrl() {
    if (!_hasVisibleSignatureStroke) return '';
    ...
    for (final point in _signaturePoints) {
      if (point == null) {
        ...
        buffer.write(
            'M ${point.dx.toStringAsFixed(1)} ${point.dy.toStringAsFixed(1)} ');
        penDown = true;
      } else {
        buffer.write(
            'L ${point.dx.toStringAsFixed(1)} ${point.dy.toStringAsFixed(1)} ');
      }
    }

    buffer.write(
        "' stroke='#1e40af' stroke-width='2.5' fill='none' stroke-linecap='round'/>");
    buffer.write('</svg>');
  ...
  Future<void> _handleSubmit() async {
    ...
    if (!_signatureCaptured) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Please provide a customer signature before submitting.'),
    ...
    final success = await context.read<WorkOrderProvider>().completeJob(
          ...
          photoOriginalFileName: widget.uploadedPhotoName,
        );
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          context.read<WorkOrderProvider>().error ??
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: provider.isLoading && job == null
          ...
          : job == null
              ? Center(
                  child: Text(provider.error ?? 'Work order not found.'),
                )
              : Form(
                  key: _formKey,
                  child: ListView(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _summaryRow('Work Order', job.workOrderNumber,
                                  isDark: isDark),
                              const SizedBox(height: 8),
                              _summaryRow('Issue', job.title, isDark: isDark),
                              const SizedBox(height: 8),
                              _summaryRow('Location', job.formattedLocation,
                                  isDark: isDark),
                            ],
                          ),
                        ),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  _statItem(Icons.timer_outlined, 'Time Spent',
                                      _formatDuration(widget.elapsedSeconds),
                                      isDark: isDark),
                                  const Spacer(),
                                  _statItem(Icons.speaker_notes_outlined,
                                      'Field Notes', '${widget.noteCount}',
                                      isDark: isDark),
                                  const Spacer(),
                                  _statItem(Icons.photo_library_outlined,
                            ...
                            children: [
                              ...
                              TextFormField(
                                ...
                                decoration: const InputDecoration(
                                  ...
                                  hintText:
                                      'Enter the name of the person signing off',
                              ...
                              Container(
                                ...
                                child: ClipRRect(
                                  ...
                                  child: GestureDetector(
                                    onPanStart: (details) => _addSignaturePoint(
                                        details.localPosition),
                                    onPanUpdate: (details) =>
                                        _addSignaturePoint(
                                            details.localPosition),
                                    onPanEnd: (_) => _endSignatureStroke(),
                                    onPanCancel: _endSignatureStroke,
                                    child: Stack(
                                      children: [
                                        if (!_signatureCaptured)
                                          Center(
